...
namespace Easy2D.Game
{
    ...
    public abstract class Game
    {
        ...
        private volatile bool isMultiThreaded;
        private volatile int updateThreadID = INACTIVE_THREAD;
            ...
            set
            {
                if (value != isMultiThreaded)
                {
                    ...
                    if (isMultiThreaded)
                    {
                        System.Threading.ThreadPool.QueueUserWorkItem(new ((o) =>
                        {
                            updateThreadID = System.Threading.Thread.CurrentThread.ManagedThreadId;
                            ...
                            updateThreadID = INACTIVE_THREAD;
                        }));
                    }
                }
            }
        ...
        public void Update(double delta)
        {
            //Return if multithreaded update and current update is comming from the main thread;
            if (updateThreadID != INACTIVE_THREAD && System.Threading.Thread.CurrentThread.ManagedThreadId != updateThreadID)
                return;
