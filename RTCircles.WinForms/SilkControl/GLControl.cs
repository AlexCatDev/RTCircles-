...
namespace Silk.NET.WinForms
{
    ...
    public class GLControl : Control
    {
        ...
        {
            get => _glControlSettings.Profile;
            set
            {
                if (value != Profile)
                {
        ...
        {
            get => _glControlSettings.APIVersion;
            set
            {
                if (value != APIVersion)
                {
        ...
        [Browsable(false)]
        public OpenGLES.GL OpenGLES { get; private set; }

        [Browsable(false)]
        public IView View => _nativeWindow;
        ...
        [Browsable(false)]
        private unsafe bool IsNativeWindowFocused
        {
            get
            {
                ...
                var glfw = Windowing.Glfw.GlfwWindowing.GetExistingApi(_nativeWindow);

                return glfw.GetWindowAttrib((GLFW.WindowHandle*)_nativeWindow.Native.Glfw.Value, GLFW.WindowAttributeGetter.Focused);
            }
            set
            {
                if (_nativeWindow == null)
                ...
                {
                    var glfw = Windowing.Glfw.GlfwWindowing.GetExistingApi(_nativeWindow);
                    glfw.FocusWindow((GLFW.WindowHandle*)_nativeWindow.Native.Glfw.Value);
                }
            }
        }
        ...
        protected override void OnHandleCreated(EventArgs e)
        {
            ...
            if (Focused || IsNativeWindowFocused)
            {
        ...
        private void CreateNativeWindow(GLControlSettings glControlSettings)
        {
            ...
            WindowOptions options = glControlSettings.ToWindowOptions();
            ...
            OpenGLES = _nativeWindow.CreateOpenGLES();
        ...
        private unsafe void NonportableReparent(IWindow nativeWindow)
        {
            if (RuntimeInformation.IsOSPlatform(OSPlatform.Windows))
            {
                IntPtr hWnd = nativeWindow.Native.Win32.Value.Hwnd;
