...
namespace Silk.NET.WinForms
{
    ...
    public class GLControlSettings
    {
        ...
        public WindowOptions ToWindowOptions()
            => new WindowOptions
            {
                PreferredStencilBufferBits = StencilBits,
                PreferredDepthBufferBits = DepthBits,
                PreferredBitDepth = new Maths.Vector4D<int>(RedBits, GreenBits, BlueBits, AlphaBits),
                Samples = NumberOfSamples,

                API = new GraphicsAPI(API, Profile, Flags, FixupVersion(APIVersion)),
                IsEventDriven = IsEventDriven,

                Size = new Maths.Vector2D<int>(1,1),
                Title = " ",
        ...
        private static APIVersion FixupVersion(Version version)
            => new APIVersion(
